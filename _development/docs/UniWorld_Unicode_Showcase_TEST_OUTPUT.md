...
These should be **single grapheme clusters** (ExtPict + ZWJ + ExtPict, etc.).

#### 9.4 Regional Indicator Flags

- `🇺🇳` (UN) `🇺🇸` (US) `🇨🇦` (Canada) `🇧🇷` (Brazil) `🇯🇵` (Japan) `🇰🇷` (Korea)
- Sequence of flags: `🇺🇸🇨🇦🇲🇽🇧🇷🇯🇵`

Regional indicators must pair: `RI RI → one cluster`, so the list above forms flag clusters without splitting inside each flag.
